// legolas - the one and only archer from LOTR

#include <iostream>
#include <fstream>
#include <vector>
#include "/Users/caw/Libraries/nauty26b05/nauty.h"
...
{
	// Check the command line arguments for peace of mind
	if (argc != 3)
	{
		cerr << "usage: legolas <arrow config> <graph file list>" << endl;
		return -1;
	}

	// Default arrow configuration parameters
	int numColors = -1;
	bool edgeColorFlag = false;
	bool vertexFolorFlag = false;

	cout << "*** Parsing arrow parameter file..." << endl;
	string line;
	vector<string> lines;
	...
	// if e/v in lines.get(0), set flag
	// save last integer to lines.get(1) as int
	// parse through next numColors lines and save the parameters (either n for Kn or g6 string for arbitrary graph)

	paramFile.close();

	cout << "*** Running arrow test..." << endl;
